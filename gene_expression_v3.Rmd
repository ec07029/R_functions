...
```{r folder}
folder <- "~/mount/chuk/Nynrin/HyperTRIBE/Analysis/OCIAML_HyperTRIBE_Analysis/"
output.dir <-"~/mount/chuk/Nynrin/HyperTRIBE/Analysis/OCIAML_HyperTRIBE_Analysis/output/"
```

Import data. Need to be RDS object in order for downstream stuff to work.

```{r data lsc}
reads <- readRDS("~/mount/chuk/Nynrin/HyperTRIBE/Project_10932_B_Nynrin_hypertribe/PITT_0483/rnaseq_read_count_entrez_id.rds")

colnames(reads) <- gsub("Sample_T_ctrl1_IGO_10932_B_11.split.bam", "Sample_T_Ctrl1_IGO_10932_B_11.split.bam", colnames(reads))
```


```{r filter}
group.one <- "Sample_O_ctrl" # bottom of the log2FC(group.two/group.one)
group.two <- "Sample_O_ADAR"
