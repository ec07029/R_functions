...
# setwd("/Users/karen/mount/chuk/Z138_MSI2_Chiara/Project_10874_B/JAX_0444/trim_galore/")

condition.mut <- "MSI2ADAR-Control"
condition.mig <- "MIGR1-Control"

df1.raw <- read.xlsx( "~/mount/chuk/Z138_MSI2_Chiara/Project_10874_B/JAX_0444/trim_galore/Z138_MSI2_Chiara_snp_counts.xlsx" )
df1.subset <- subset(df1.raw, dbsnp=="")

mut.index <- which( grepl(condition.mut, colnames(df1.subset)) )
mig.index <- which( grepl(condition.mig, colnames(df1.subset)) )

df1 <- cbind( df1.subset[,1:9], df1.subset[, c(mut.index, mig.index)])
...
mut.index <- which( grepl(condition.mut, colnames(ref.counts)) )
mig.index <- which( grepl(condition.mig, colnames(ref.counts)) )
...
lrt.res <- mclapply( 1:nrow(df1), function( i ){ 
       ...
       fit1 <- mle.custom.h1( ref[ mut.index ], alt[ mut.index ], 
                              ref[ c(mig.index) ], alt[ c(mig.index) ] )
...
write.csv(df1, paste0("../output/Z138_", condition.mut, "_vs_", condition.mig, "_snp_counts_significance.csv"), row.names = FALSE)

# Add gene FPKM and # edits
gene.num.edits <- table( df1$entrez.id )
se <- readRDS( "/data/leslie/chuk/Z138_MSI2_Chiara/Project_10874_B/JAX_0444/trim_galore/rnaseq_read_count_entrez_id.rds" )
fpkm.mat <- fpkm( DESeqDataSet( se, ~1 ) ) # originally ~condition but R gives error cause it was never defined -Karen

mut.index <- which( grepl(condition.mut, colnames(fpkm.mat)) )
mig.index <- which( grepl(condition.mig, colnames(fpkm.mat)) )

fpkm.mat <- data.frame( mut.fpkm = rowMeans( fpkm.mat[,mut.index]),
                        mig.fpkm = rowMeans( fpkm.mat[,mig.index]) )
colnames(fpkm.mat) <- c( paste0(condition.mut, ".fpkm"),
                         paste0(condition.mig, ".fpkm"))
...
write.csv(df1, paste0("../output/Z138_", condition.mut, "_vs_", condition.mig, "_snp_counts_significance_fpkm.csv"), row.names = FALSE)
