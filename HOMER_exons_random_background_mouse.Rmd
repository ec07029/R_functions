...
```{r setwd}
folder <- "~/mount/chuk/RBM15_Dart-Seq/Project_11379_B/Analysis/HOMER/MP1/"
save.folder <- "~/mount/chuk/RBM15_Dart-Seq/Project_11379_B/Analysis/HOMER/MP1/"
```

Import data.

```{r import}
df <- read.csv("~/mount/chuk/RBM15_Dart-Seq/Project_11379_B/JAX_0488/RBM15_Dart-seq_MP1_B_vs_MP1_M_snp_counts_significance.csv")

fpkm.index <- which( grepl("fpkm", colnames(df)) )
...
```{r utr}
utr3 <- exonsBy(txdb) # Get exon genomic coordinates. Keep variable name same to avoid changing it all over the script.
...
writeFasta(hypertribe.cluster.sequence,
           paste0(save.folder, "RBM15_Dart-Seq_MP1_plusminus_100bp_window.fa"))
writeFasta(hypertribe.cluster.sequence.background,
           paste0(save.folder, "RBM15_Dart-Seq_MP1_RANDOM_BACKGROUND.fa"))
